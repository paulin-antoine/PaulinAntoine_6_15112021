...
exports.signup = (req, res,next) => {
    ...
    if (!emailValidation.validate(req.body.email)) {
        return res.status(403).json({ message: 'Le format email n\'est pas valide !'});
        };
    ...
    bcrypt.hash(req.body.password, 10)
      ...
      .catch(error => res.status(500).json({error}));
        ...
        return res.status(403).json({message: 'Le mot de passe doit contenir au minimum 6 caractères'})
    };
}
